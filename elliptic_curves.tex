...
\begin{proof}
	...
	the composition $\psi\circ\phi: E \to \proj^1$ is a map of degree between
	smooth curves and hence an isomorphism (\comment{reference}).
	This contradicts the fact that $E$ has genus 1 and $\proj^1$ has genus 0.
	(\comment{reference}). Hence $C$ is smooth, so again by
	(\comment{reference}), we have that the degree 1 map $\phi: E \to \C$ is
...
\begin{proof}
	...
	and so a morphism.
	\comment{Not sure about this proof...}
\end{proof}
